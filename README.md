...
1. На GitHub проверьте версию в `public/manifest.json`: для этого выпуска нужна **1.6.0**. Пока изменения ждут слияния, выберите ветку `codex/local-pdf`, скачайте её через **Code → Download ZIP** и распакуйте архив.
...
- Для перевода страницы источник определяется между английским и русским по фрагментам. Фрагменты уже на выбранной цели не меняются; при смешанных языках внутри одного текстового узла возможны неточные результаты.
...
- Схема хранения v2 автоматически читает историю и словарь из v1. Экспорт создаёт резервную копию v2 с прогрессом карточек; импорт принимает старые v1 и новые v2, сохраняя более позднюю оценку совпадающей пары.
