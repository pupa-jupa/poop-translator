...
- тёмная сливово-графитовая палитра с насыщенными розовыми акцентами;
...
Браузерный smoke запускает видимый Chromium с временным профилем; на Linux нужен графический сеанс или Xvfb. Переводчик в popup подменяется тестовой заглушкой: сценарий проверяет интерфейс, импорт, масштаб и взаимодействие модулей, но не качество настоящей модели. Отдельная команда `npm run smoke:real` не ставит заглушки и завершается ошибкой, если модель не загрузилась. CI выполняет тесты, типы, сборку и файловый smoke. Подробнее: [проверки и ручная приёмка](docs/testing.md).
